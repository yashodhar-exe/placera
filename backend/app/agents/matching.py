...
from app.models import Student, JobDrive, EligibilityResult, CandidateMatch, StudentSkill, Resume
...
class MatchingAgent:
    ...
    async def run_matching(self, db: AsyncSession, drive_id: int):
        ...
        for er in eligible_results:
            ...
            # Merge with resume skills just in case
            student_skill_names = list(set(student_skill_names + list(skill_evidence_map.keys())))
            
            matched = set(jd_skills).intersection(set(student_skill_names))
            missing = set(jd_skills) - set(student_skill_names)
            ...
            # Format Evidence
            evidence_json = {skill: skill_evidence_map.get(skill, "Supported by profile data.") for skill in matched}
